...
export const MainSidebar = ({ isOpen, toggleOpen }: MainSidebarProps) => {
    ...
    return (
        <aside className={`relative flex flex-col h-full bg-white dark:bg-canvas-dark border-r border-slate-200 dark:border-slate-800/85 shrink-0 z-30 transition-[width] duration-300 ease-in-out ${isOpen ? 'w-40' : 'w-12'}`}>
            ...
            <nav className="py-2 px-1.5 space-y-1.5 flex-1 overflow-hidden">
                <button className="w-full flex items-center h-9 rounded-xl bg-blue-600 text-white font-semibold text-[11px] transition-all shadow-md shadow-blue-500/20 overflow-hidden" title={t('sidebar.map')}>
                ...
                <button className="w-full flex items-center h-9 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/50 font-medium text-[11px] transition-all overflow-hidden" title={t('sidebar.counties')}>
                    <div className="w-9 h-full flex items-center justify-center shrink-0">
                ...
                <button className="w-full flex items-center h-9 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/50 font-medium text-[11px] transition-all overflow-hidden" title={t('sidebar.rankings')}>
                    <div className="w-9 h-full flex items-center justify-center shrink-0">
                        <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
                    </div>
                    <span className={`ml-1 whitespace-nowrap transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>{t('sidebar.rankings')}</span>
                </button>
            </nav>
        </aside>
    );
