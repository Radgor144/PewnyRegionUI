...
export const CountyDetailsPanel = ({ county, isOpen, setIsOpen }: CountyDetailsPanelProps) => {
    ...
    return (
        <div className="absolute top-12 bottom-0 right-0 z-40 flex pointer-events-none overflow-visible transition-all duration-300">
            <aside className={`h-full bg-white dark:bg-[#111827] border-l border-slate-200 dark:border-slate-800/80 shadow-2xl transition-[width] duration-300 ease-in-out overflow-hidden pointer-events-auto flex flex-col ${isOpen ? 'w-[400px]' : 'w-0 border-l-0'}`}>
                <div className={`w-[400px] shrink-0 h-full flex flex-col transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                    <div className="flex flex-col justify-center px-4 h-14 border-b border-slate-200 dark:border-slate-800/80 shrink-0 overflow-hidden">
