            ...
            {results.length > 0 && (
                <ul className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-panel-dark rounded-xl shadow-xl overflow-hidden border border-slate-200 dark:border-slate-700 backdrop-blur-md">
                    {results.map((feature, index) => (
                        <li key={getTeryt(feature.properties) ?? index} onClick={() => zoomToFeature(feature)} className="cursor-pointer px-3 py-2 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border-b border-slate-100 dark:border-slate-700/50 last:border-0">
