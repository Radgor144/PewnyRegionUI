    ...
    const searchUI = (
        <div className="relative w-64 z-[1000]">
            ...
            <input
                ...
                className="w-full pl-8 pr-3 py-1 rounded-lg border border-slate-200 dark:border-slate-700/80 bg-slate-100 dark:bg-panel-dark/50 text-xs font-medium text-slate-900 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-400 outline-none focus:border-blue-500 dark:focus:border-blue-500 focus:bg-white dark:focus:bg-panel-dark transition-all"
            />

            {results.length > 0 && (
                <ul className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-panel-dark rounded-lg shadow-xl overflow-hidden border border-slate-200 dark:border-slate-700 backdrop-blur-md">
                    {results.map((feature, index) => (
                        <li key={getTeryt(feature.properties) ?? index} onClick={() => zoomToFeature(feature)} className="cursor-pointer px-3 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border-b border-slate-100 dark:border-slate-700/50 last:border-0">
